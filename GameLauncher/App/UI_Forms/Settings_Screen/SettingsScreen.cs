...
namespace GameLauncher.App.UI_Forms.Settings_Screen
{
    public partial class SettingsScreen : Form
    {
        ...
        private void SetVisuals()
        {
            ...
            SettingsLauncherVersion.Text = "เวอร์ชั่น: v" + Application.ProductVersion;
        ...
        private void IsChangedCDNDown()
        {
            ...
            {
                SettingsCDNText.Text = "CDN: กำลังทดสอบ";
                    ...
                    if (!Application.OpenForms[this.Name].IsDisposed)
                    {
                        switch (APIChecker.CheckStatus(((CDNList)SettingsCDNPick.SelectedItem).Url + "/index.xml", 10))
                        {
                            case APIStatus.Online:
                                ...
                                {
                                    SettingsCDNText.Text = "CDN: ออนไลน์";
                            ...
                            default:
                                SettingsCDNText.SafeInvokeAction(() =>
                                {
                                    SettingsCDNText.Text = "CDN: ออฟไลน์";
        ...
        private void PingAPIStatus()
        {
            ...
            {
                SettingsMainSrvText.Text = "เซิร์ฟเวอร์ API: ออนไลน์";
            ...
            else
            {
                ...
                {
                    SettingsMainSrvText.Text = "เซิร์ฟเวอร์ API: " + Strings.Truncate(APIChecker.StatusStrings(VisualsAPIChecker.UnitedSC), 32);
            ...
            {
                SettingsMainCDNText.Text = "เซิร์ฟเวอร์ API ๅ: ออนไลน์";
                SettingsMainCDNText.ForeColor = Theming.Sucess;
            }
            else
            {
                SettingsMainCDNText.ForeColor = Theming.Warning;
                if (VisualsAPIChecker.CarbonSL && !VisualsAPIChecker.CarbonCDNL) { SettingsMainCDNText.Text = "เซิร์ฟเวอร์ API: แสดงผลเฉพาะ รายการเซิร์ฟเวอร์"; }
                else if (!VisualsAPIChecker.CarbonSL && VisualsAPIChecker.CarbonCDNL) { SettingsMainCDNText.Text = "เซิร์ฟเวอร์ API: แสดงผลเฉพาะ CND"; }
                else
                {
                    SettingsMainCDNText.Text = "เซิร์ฟเวอร์ API: " + Strings.Truncate(APIChecker.StatusStrings(VisualsAPIChecker.CarbonSC), 32);
            ...
            if (VisualsAPIChecker.CarbonAPITwo())
            {
                SettingsBkupSrvText.Text = "เซิร์ฟเวอร์ API 2: ออนไลน์";
                SettingsBkupSrvText.ForeColor = Theming.Sucess;
            }
            else
            {
                SettingsBkupSrvText.ForeColor = Theming.Warning;
                if (VisualsAPIChecker.CarbonTwoSL && !VisualsAPIChecker.CarbonTwoCDNL) { SettingsBkupSrvText.Text = "เซิร์ฟเวอร์ API 2: แสดงผลเฉพาะ รายการเซิร์ฟเวอร์"; }
                else if (!VisualsAPIChecker.CarbonTwoSL && VisualsAPIChecker.CarbonTwoCDNL) { SettingsBkupSrvText.Text = "เซิร์ฟเวอร์ API 2: แสดงผลเฉพาะ CND"; }
                else
                {
                    SettingsBkupSrvText.Text = "เซิร์ฟเวอร์ API 2: " + Strings.Truncate(APIChecker.StatusStrings(VisualsAPIChecker.CarbonTwoSC), 32);
        ...
        private void RememberLastCDN()
        {
            if (CDNListUpdater.CleanList != null)
            {
                ...
                {
                    /* Last Selected CDN */
                    Log.Core("ตั้งค่ารายการ CND: กำลังตรวจสอบ...");
                    Log.Core("ตั้งค่ารายการ CND: ตั้งค่าเซิร์ฟเวอร์ CND ในรายการแล้ว");
                    Log.Core("ตั้งค่ารายการ CND: ตรวจสอบหากเซิร์ฟเวอร์ตั้งค่าในไฟล์ INI");
